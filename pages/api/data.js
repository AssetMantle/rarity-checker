...
export async function fetchNftData2() {
  let nftList = [];
  let response;
  ...
  while (page_number <= 5) {
    ...
    lastAssetIDUint8Array =
      response?.list[response?.list?.length - 1]?.key?.assetID?.hashID?.iDBytes;
  ...
  //   let finalList = filterProperties(dummyData?.list);
...
//   const hashIdUint8Array = new Uint8Array(
//     base64url.toBuffer("ABgCuZ4wyhIqIwxQXyN1Bw8uyVm/NBBYmmQMV/yTCOY=")
//   );

//   let test = Base64.fromUint8Array(hashIdUint8Array);
...
//       //   countTotal: true,
//       //   key: new Uint8Array(),
//       //   key: { assetID: { hashID: { iDBytes: hashIdUint8Array } } },
//     });
...
//   let finalList = filterProperties(response?.list);
//   //   let finalList = filterProperties(dummyData?.list);
...
const filterProperties = (list) => {
  ...
  if (list && list?.length > 0) {
    ...
    for (const nft of list) {
      ...
      for (const mutableProperty of nft?.mappable?.asset?.mutables?.propertyList
        ?.anyProperties) {
        // check for mesa property
        if (mutableProperty?.mesaProperty) {
          ...
          // increment count of property type
          rarity[mutableProperty?.mesaProperty?.iD?.keyID?.iDString][cleanData]
            .count++;
        ...
        if (mutableProperty?.metaProperty) {
          ...
          // filter key:value which is not undefined inside data
          // **return obj if you want data type
          const cleanData = Object.entries(mutableProperty?.metaProperty?.data)
            .filter(([key, value]) => value !== undefined)
            .reduce((obj, [key, value]) => {
              obj[key] = value;
              return value;
